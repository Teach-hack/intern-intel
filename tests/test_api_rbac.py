...
def test_user_cannot_access_admin_route(client: TestClient, db_session: Session, normal_user_token: dict[str, str]) -> None:
...
def test_admin_can_access_admin_route(client: TestClient, db_session: Session, admin_token: dict[str, str]) -> None:
...
def test_admin_cannot_delete_last_admin(client: TestClient, db_session: Session, admin_token: dict[str, str]) -> None:
    """Test that the last admin cannot be deleted."""
    headers = {"Authorization": f"Bearer {admin_token['access_token']}"}
    
    # Ensure there's only 1 admin
    admins = db_session.execute(select(User).where(User.role == UserRole.ADMIN)).scalars().all()
    assert len(admins) == 1
...
def test_admin_cannot_demote_last_admin(client: TestClient, db_session: Session, admin_token: dict[str, str]) -> None:
    ...
    payload = {"role": "USER"}
    response = client.patch(f"/api/v1/admin/users/{admin_id}", json=payload, headers=headers)
